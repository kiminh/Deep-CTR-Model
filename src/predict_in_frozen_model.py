...
def _make_feacture_dict(column_config):
    feature_dict = {}
...
def get_pwc_examples(sp, query_file, max_len):
    result = []
    for s in codecs.open(query_file, 'r', encoding='utf8'):
        ...
        pwc_feature_dict = _make_pwc_feacture_dict(
            ad_id, total_rank, ad_headline)
        example = tf.train.Example(
            features=tf.train.Features(feature=pwc_feature_dict))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--column_config_file", type=string, required=True)
    ...
    examples = get_pwc_examples(sp, args.query_file, args.max_len)
